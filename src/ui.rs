...
pub fn render<B: Backend>(app: &mut App, frame: &mut Frame<'_, B>) {
    ...
    render_verse(app, frame, chunks[6])
}
...
fn render_verse<B: Backend>(app: &mut App, frame: &mut Frame<'_, B>, rect: Rect) {
    let verse_title = app.verse_title();
    let verse = Block::default()
        .title(verse_title)
        .title_alignment(Alignment::Center)
        .borders(Borders::ALL)
        .border_type(BorderType::Rounded);

    let rect_inside_block = verse.inner(rect);
    frame.render_widget(verse, rect);
    ...
    render_verse_text(app, frame, chunks[0]);
    render_footnotes(app, frame, chunks[1]);
}

fn render_verse_text<B: Backend>(app: &mut App, frame: &mut Frame<'_, B>, rect: Rect) {
    let text = Paragraph::new(app.verse_text())
        .scroll((app.text_scroll, 0))
        .wrap(Wrap { trim: false });
    frame.render_widget(text, rect);
}

fn render_footnotes<B: Backend>(_app: &mut App, frame: &mut Frame<'_, B>, rect: Rect) {
    let footnotes = Block::default()
        .title("Footnotes")
        .title_alignment(Alignment::Center)
        .borders(Borders::TOP);
